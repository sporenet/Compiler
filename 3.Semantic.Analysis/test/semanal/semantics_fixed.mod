...
procedure If(p1, p2);
begin
  if (p1 # p2) then         // modified, pass (boolean expression expected.)
    return
  else
    return
  end;
...
var a,b,c,d: integer;
    e,f,g,h: boolean;
begin
	a := a+(b+(c+(d+(d+a*c/d)/d*d/c)*b+b-b/(c-d*d/a-b))-3-4-5-3-4-2-3/3/4/5/3/4/4);
	b := (a*b/c+d)-(3*0/0*4)+((c*a+d-(d-c-b-a*d))-c*(a+b-3/0));
	f := true && false || true && !!!!!!!!!!(true && f && g || (h # e));
	while ((f#g)#h) do end;
  b:=x
end Complicated;
